function res = WENO5resAdv_Y(w,flux,dflux,dy,N)
% *************************************************************************
...
W=cat(1,w(1,:,:),w(1,:,:),w(I-2,:,:),w(N,:,:),w(N,:,:),w(N,:,:));
hn=zeros(size(W));
hp=zeros(size(W));
...
% Numerical Flux at cell boundary, $u_{i+1/2}^{+}$;
hp(I,:,:) = w0p.*( -umm + 5*um + 2*u  )/6 + ...
            w1p.*( 2*um + 5*u  - up   )/6 + ...
            w2p.*(11*u  - 7*up + 2*upp)/6;

%% Compute finite difference residual term, df/dy.
res = (hp(I,:,:)+hn(I,:,:)-hp(I-1,:,:)-hn(I-1,:,:))/dy;