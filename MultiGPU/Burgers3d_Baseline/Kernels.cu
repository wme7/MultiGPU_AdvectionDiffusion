  ...
  const REAL dx)
{
  ...
  if (j>=3 && j<ny-3 && k>=kstart && k<kstop) 
  {
    o=pitch*j+pitch*ny*k;
    // Old resulst arrays
    fu_old=0;
  ...
  const REAL dy)
{
  ...
  // Indexes
  unsigned int i, j, k, o, xy; xy=pitch*ny;
  ...
  // Compute only for internal nodes
  if (i>=3 && i<nx-3 && k>=kstart && k<kstop) 
  {
    o=i+xy*k;
    // Old resulst arrays
    fu_old=0;
    ...
    f1mm= 0.5*(Flux(u[    o    ]) + fabs(u[    o    ])*u[    o    ]); // node(i-2)
    f1m = 0.5*(Flux(u[ o+pitch ]) + fabs(u[ o+pitch ])*u[ o+pitch ]); // node(i-1)
    f1  = 0.5*(Flux(u[o+2*pitch]) + fabs(u[o+2*pitch])*u[o+2*pitch]); // node( i )     imm--im--i--ip--ipp--ippp
    f1p = 0.5*(Flux(u[o+3*pitch]) + fabs(u[o+3*pitch])*u[o+3*pitch]); // node(i+1)
    
    g1mm= 0.5*(Flux(u[ o+pitch ]) - fabs(u[ o+pitch ])*u[ o+pitch ]); // node(i-1)
    g1m = 0.5*(Flux(u[o+2*pitch]) - fabs(u[o+2*pitch])*u[o+2*pitch]); // node( i )     imm--im--i--ip--ipp--ippp
    g1  = 0.5*(Flux(u[o+3*pitch]) - fabs(u[o+3*pitch])*u[o+3*pitch]); // node(i+1)
    g1p = 0.5*(Flux(u[o+4*pitch]) - fabs(u[o+4*pitch])*u[o+4*pitch]); // node(i+2)
...
/*****************/
/* Compute dH/dz */ // <==== parallel strategy: compute serialy by rows or by columns!
/*****************/
  ...
  const REAL dz)
{
  ...
  // Indexes
  unsigned int i, j, k, o, xy, nk; xy=pitch*ny; nk = kstop-kstart;
  ...
  {
    o = i+pitch*j+xy*kstart;
    // Old resulst arrays
    f1mm= 0.5*(Flux(u[o-3*xy]) + fabs(u[o-3*xy])*u[o-3*xy]); // node(i-2)
    f1m = 0.5*(Flux(u[o-2*xy]) + fabs(u[o-2*xy])*u[o-2*xy]); // node(i-1)
    f1  = 0.5*(Flux(u[ o-xy ]) + fabs(u[ o-xy ])*u[ o-xy ]); // node( i )     imm--im--i--ip--ipp--ippp
    f1p = 0.5*(Flux(u[  o   ]) + fabs(u[  o   ])*u[  o   ]); // node(i+1)
    f1pp= 0.5*(Flux(u[ o+xy ]) + fabs(u[ o+xy ])*u[ o+xy ]); // node(i+1)
    
    g1mm= 0.5*(Flux(u[o-2*xy]) - fabs(u[o-2*xy])*u[o-2*xy]); // node(i-1)
    g1m = 0.5*(Flux(u[ o-xy ]) - fabs(u[ o-xy ])*u[ o-xy ]); // node( i )     imm--im--i--ip--ipp--ippp
    g1  = 0.5*(Flux(u[  o   ]) - fabs(u[  o   ])*u[  o   ]); // node(i+1)
    g1p = 0.5*(Flux(u[ o+xy ]) - fabs(u[ o+xy ])*u[ o+xy ]); // node(i+2)
    g1pp= 0.5*(Flux(u[o+2*xy]) - fabs(u[o+2*xy])*u[o+2*xy]); // node(i+2)
    ...
    {
      // Compute and split fluxes
      f1pp= 0.5*(Flux(u[o+(k+2)*xy]) + fabs(u[o+(k+2)*xy])*u[o+(k+2)*xy]); // node(i+2)
      g1pp= 0.5*(Flux(u[o+(k+3)*xy]) - fabs(u[o+(k+3)*xy])*u[o+(k+3)*xy]); // node(i+3)
      
      // Reconstruct
      fu = Reconstruct1d(f1mm,f1m,f1,f1p,f1pp,g1mm,g1m,g1,g1p,g1pp);
      
      // Compute Lq = dH/dz
      Lu[o+xy*k]-=(fu-fu_old)/dz; // dudz
      // Lu[o+xy*k]= o+xy*k; // <-- debuging tool
...
extern "C" void Compute_Adv_x_Async(dim3 numBlocks, dim3 threadsPerBlock, cudaStream_t aStream, 
  unsigned int pitch, unsigned int Nx, unsigned int Ny, unsigned int _Nz, unsigned int kstart, unsigned int kstop, REAL dx, 
  REAL *u, REAL *Lu)
{
  Compute_dF<<<numBlocks,threadsPerBlock,0,aStream>>>(u,Lu,pitch,Nx,Ny,_Nz,kstart,kstop,LOOP,dx);
}

extern "C" void Compute_Adv_y_Async(dim3 numBlocks, dim3 threadsPerBlock, cudaStream_t aStream, 
  unsigned int pitch, unsigned int Nx, unsigned int Ny, unsigned int _Nz, unsigned int kstart, unsigned int kstop, REAL dy,
  REAL *u, REAL *Lu)
{
  Compute_dG<<<numBlocks,threadsPerBlock,0,aStream>>>(u,Lu,pitch,Nx,Ny,_Nz,kstart,kstop,LOOP,dy);
}

extern "C" void Compute_Adv_z_Async(dim3 numBlocks, dim3 threadsPerBlock, cudaStream_t aStream, 
  unsigned int pitch, unsigned int Nx, unsigned int Ny, unsigned int _Nz, unsigned int kstart, unsigned int kstop, REAL dz,
  REAL *u, REAL *Lu)
{
  Compute_dH<<<numBlocks,threadsPerBlock,0,aStream>>>(u,Lu,pitch,Nx,Ny,_Nz,kstart,kstop,LOOP,dz);
}

extern "C" void Compute_sspRK3Async(dim3 numBlocks, dim3 threadsPerBlock, cudaStream_t aStream, 
  unsigned int step, unsigned int pitch, unsigned int Nx, unsigned int Ny, unsigned int _Nz, REAL dt, 
