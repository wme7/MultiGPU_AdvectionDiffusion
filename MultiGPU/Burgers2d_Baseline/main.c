...
{
	REAL tEnd, CFL, L, W;
    unsigned int Nx, Ny, blockX, blockY;
    ...
    {
        printf("Usage: %s tEnd CFL L W Nx Ny block_x block_y\n", argv[0]);
        exit(1);
    }
	...
	// Define Constanst
    REAL dx = L/(Nx-1);	// dx, cell size
    REAL dy = W/(Ny-1);	// dy, cell size
    unsigned int _Ny = Ny/numberOfProcesses;	// Decompose along the z-axis
    unsigned int  NY = Ny+2*RADIUS;	// Extended domain
    unsigned int _NY =_Ny+2*RADIUS;	// subdomain size
    unsigned int dt_size= sizeof(REAL);	// Data size
	...
	unsigned int blocksInY = getBlock(_NY,blockY);
    ...
    dim3 threadsPerBlock_Y(1,blockY);
    dim3 numBlocks_Y(1,blocksInY);

    // Sweep in y-direction
    dim3 threadsPerBlock_X(blockX,1);
    dim3 numBlocks_X(blocksInX,1);
	...
	while (t < tEnd)
	{
		...
		for (unsigned int step = 1; step <= 3; step++) // 3 runge kutta steps!!
		{
			...
			if (rank < numberOfProcesses-1)
			{
				...
				Compute_Adv_x_Async(numBlocks_Y, threadsPerBlock_Y, r_send_stream, pitch, Nx, _NY, jstart, jstop, dx, d_s_u, d_s_Lu);
				Compute_Adv_y_Async(numBlocks_X, threadsPerBlock_X, r_send_stream, pitch, Nx, _NY, jstart, jstop, dy, d_s_u, d_s_Lu);
			...
			if (rank > 0)
			{
				...
				Compute_Adv_x_Async(numBlocks_Y, threadsPerBlock_Y, l_send_stream, pitch, Nx, _NY, jstart, jstop, dx, d_s_u, d_s_Lu);
				Compute_Adv_y_Async(numBlocks_X, threadsPerBlock_X, l_send_stream, pitch, Nx, _NY, jstart, jstop, dy, d_s_u, d_s_Lu);
			...
			if (rank == 0)
			{
				...
				Compute_Adv_x_Async(numBlocks_Y, threadsPerBlock_Y, computeStream, pitch, Nx, _NY, jstart, jstop, dx, d_s_u, d_s_Lu);
				Compute_Adv_y_Async(numBlocks_X, threadsPerBlock_X, computeStream, pitch, Nx, _NY, jstart, jstop, dy, d_s_u, d_s_Lu);
			}
			...
			Compute_sspRK3Async(numBlocks2D, threadsPerBlock2D, computeStream, step, pitch, Nx, _NY, dt, d_s_u, d_s_uo, d_s_Lu);
		}
	}
	...
	if (rank == 0)
	{
		...
		PrintSummary("Burgers-2D MPI-GPU-WENO5", "Pitched Memory", compute_timer, HtD_timer, DtH_timer, gflops, it, Nx, NY);
	}
