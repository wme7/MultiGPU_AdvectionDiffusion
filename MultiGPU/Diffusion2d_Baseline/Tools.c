...
void Init_domain(int IC, REAL *u0, REAL dx, REAL dy, unsigned int nx, unsigned int ny)
{
    ...
    case 3: {
      // Spherical discontinuity
      for (j= 0; j < ny; j++) {
        for (i= 0; i < nx; i++) {
          o = i+nx*j;
          if ( sqrtf((i*1./(nx-1)-0.5)*(i*1./(nx-1)-0.5)+(j*1./(ny-1)-0.5)*(j*1./(ny-1)-0.5)) < 0.2 ) {
            u0[o]=1.0;
          } else {
            u0[o]=0.0;
          }
...
void PrintSummary(const char* kernelName, const char* optimization,
    ...
    float gflops, int computeIterations, unsigned int nx, unsigned int ny)
{
    printf("===========================%s=======================\n", kernelName);
