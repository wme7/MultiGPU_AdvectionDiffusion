...
void PrintSummary(const char* kernelName, const char* optimization, REAL computeTimeInSeconds, float gflops, REAL outputTimeInSeconds, int computeIterations, int nx, int ny);
void CalcError(REAL *u, REAL t, REAL dx, REAL dy, unsigned int nx, unsigned int ny);

/* Device wrappers */
extern "C" 
{
void InitilizeTexture(REAL *u, unsigned int nx, unsigned int ny, size_t pitch_bytes);
void CopyToConstantMemory(REAL kx, REAL ky);
void Call_Lu2d(dim3 numBlocks, dim3 threadsPerBlock, cudaStream_t aStream, unsigned int pitch, unsigned int nx, unsigned int ny, REAL *Lu);
