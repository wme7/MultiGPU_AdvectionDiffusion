...
int main(int argc, char** argv){
    ...
    while (t < tEnd)
    {
        ...
        for(step=1; step<=3; step++) {
            Call_Adv_x(numBlocksYZ,threadsPerBlockYZ,compute_stream,pitch,Nx,Ny,Nz,dx,d_u,d_Lu);
            Call_Adv_y(numBlocksXZ,threadsPerBlockXZ,compute_stream,pitch,Nx,Ny,Nz,dy,d_u,d_Lu);
    ...
    PrintSummary("Burgers-3D GPU-WENO5", "Pitched Memory", compute_timer, HtD_timer, DtH_timer, gflops, it, Nx, Ny, Nz);
