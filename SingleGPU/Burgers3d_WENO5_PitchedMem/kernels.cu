  ...
  const REAL dy)
{
  ...
  if (i>2 && i<nx-3 && k>2 && k<nz-3) {
    ...
    f1mm= 0.5*( Flux(u[i+pitch*0+xy*k]) + fabs(u[i+pitch*0+xy*k])*u[i+pitch*0+xy*k]); // node(i-2)
    f1m = 0.5*( Flux(u[i+pitch*1+xy*k]) + fabs(u[i+pitch*1+xy*k])*u[i+pitch*1+xy*k]); // node(i-1)
    f1  = 0.5*( Flux(u[i+pitch*2+xy*k]) + fabs(u[i+pitch*2+xy*k])*u[i+pitch*2+xy*k]); // node( i )     imm--im--i--ip--ipp--ippp
    f1p = 0.5*( Flux(u[i+pitch*3+xy*k]) + fabs(u[i+pitch*3+xy*k])*u[i+pitch*3+xy*k]); // node(i+1)
       
    g1mm= 0.5*( Flux(u[i+pitch*1+xy*k]) - fabs(u[i+pitch*1+xy*k])*u[i+pitch*1+xy*k]); // node(i-1)
    g1m = 0.5*( Flux(u[i+pitch*2+xy*k]) - fabs(u[i+pitch*2+xy*k])*u[i+pitch*2+xy*k]); // node( i )     imm--im--i--ip--ipp--ippp
    g1  = 0.5*( Flux(u[i+pitch*3+xy*k]) - fabs(u[i+pitch*3+xy*k])*u[i+pitch*3+xy*k]); // node(i+1)
    g1p = 0.5*( Flux(u[i+pitch*4+xy*k]) - fabs(u[i+pitch*4+xy*k])*u[i+pitch*4+xy*k]); // node(i+2)
        
    for (j = 2; j < ny-3; j++) {
        
      // Compute and split fluxes
      f1pp= 0.5*( Flux(u[i+pitch*(j+2)+xy*k]) + fabs(u[i+pitch*(j+2)+xy*k])*u[i+pitch*(j+2)+xy*k]); // node(i+2)
      g1pp= 0.5*( Flux(u[i+pitch*(j+3)+xy*k]) - fabs(u[i+pitch*(j+3)+xy*k])*u[i+pitch*(j+3)+xy*k]); // node(i+3)
      
      // Reconstruct
      fu = WENO5Zreconstruction(f1mm,f1m,f1,f1p,f1pp,g1mm,g1m,g1,g1p,g1pp);
      
      // Compute Lq = dG/dy
      Lu[i+pitch*j+xy*k]-=(fu-fu_old)/dy; // dudy
...
__global__ void Compute_Laplace(
  const REAL * __restrict__ u, 
  REAL * __restrict__ Lu,
  const unsigned int px, // pitch in x-direction   
  const unsigned int nx, 
  const unsigned int ny, 
  const unsigned int nz)
{
