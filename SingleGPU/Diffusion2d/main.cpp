...
int main(int argc, char** argv)
{
    ...
    checkCuda(cudaMalloc((void**)&d_u, sizeof(REAL)*(Nx*Ny)));
    checkCuda(cudaMalloc((void**)&d_uo,sizeof(REAL)*(Nx*Ny)));
    checkCuda(cudaMalloc((void**)&d_Lu,sizeof(REAL)*(Nx*Ny)));

    // Copy Initial Condition from host to device
    checkCuda(cudaMemcpy(d_u,h_u,sizeof(REAL)*(Nx*Ny),cudaMemcpyHostToDevice));
    ...
    {
        // Runge Kutta Step 0
        checkCuda(cudaMemcpy(d_uo,d_u,sizeof(REAL)*(Nx*Ny),cudaMemcpyDeviceToDevice));
        
        // Set memory of temporal variables to zero
        checkCuda(cudaMemset(d_Lu,0,sizeof(REAL)*(Nx*Ny)));

        // Runge Kutta Step 1
        Call_Lu2d(numBlocks2D,threadsPerBlock2D,compute_for_u,Nx,Ny,d_u,d_Lu);
        Call_RK2d(numBlocks2D,threadsPerBlock2D,compute_for_u,1,Nx,Ny,dt,d_u,d_uo,d_Lu);
        
        // Runge Kutta Step 2
        Call_Lu2d(numBlocks2D,threadsPerBlock2D,compute_for_u,Nx,Ny,d_u,d_Lu);
        Call_RK2d(numBlocks2D,threadsPerBlock2D,compute_for_u,2,Nx,Ny,dt,d_u,d_uo,d_Lu);
        
        // Runge Kutta Step 3
        Call_Lu2d(numBlocks2D,threadsPerBlock2D,compute_for_u,Nx,Ny,d_u,d_Lu);
        Call_RK2d(numBlocks2D,threadsPerBlock2D,compute_for_u,3,Nx,Ny,dt,d_u,d_uo,d_Lu);
    }
    ...
    checkCuda(cudaMemcpy(h_u,d_u,sizeof(REAL)*(Nx*Ny),cudaMemcpyDeviceToHost));
