...
int main(int argc, char** argv){
    ...
    // Synchronous Laplace
    dim3 threadsPerBlockXY(2,2);
    dim3 numBlocksXY(blocksInX,blocksInY);

    // Sweep texture in the x-direction
    dim3 threadsPerBlock_X(TILE+1,WIDTH);
    blocksInX = DIVIDE_INTO(Nx,TILE );
    blocksInY = DIVIDE_INTO(Ny,WIDTH);
    dim3 numBlocks_X(blocksInX,blocksInY);

    // Sweep texture in the y-direction
    dim3 threadsPerBlock_Y(WIDTH,TILE+1);
    blocksInX = DIVIDE_INTO(Nx,WIDTH);
    blocksInY = DIVIDE_INTO(Ny,TILE );
    dim3 numBlocks_Y(blocksInX,blocksInY);

    // Sweep texture in the z-direction
    dim3 threadsPerBlock_Z(TILE+1,1,1);
    blocksInZ = DIVIDE_INTO(Nz,TILE);
    blocksInX = DIVIDE_INTO(Nx,  1 );
    blocksInY = DIVIDE_INTO(Ny,  1 );
    dim3 numBlocks_Z(blocksInZ,blocksInX,blocksInY);
    ...
    while (t < tEnd)
    {
        ...
        for(step=1; step<=3; step++) {
            // Call_Adv_L(numBlocks_X,threadsPerBlock_X,numBlocks_Y,threadsPerBlock_Y,numBlocks_Z,threadsPerBlock_Z,
            //     compute_stream,pitch_bytes,pitch,&desc,Nx,Ny,Nz,dx,dy,dz,d_u,d_Lu); // all advection kernels together
            Call_Adv_x(numBlocks_X,threadsPerBlock_X,compute_stream,pitch_bytes,pitch,&desc,Nx,Ny,Nz,dx,d_u,d_Lu);
            Call_Adv_y(numBlocks_Y,threadsPerBlock_Y,compute_stream,pitch_bytes,pitch,&desc,Nx,Ny,Nz,dy,d_u,d_Lu);
            Call_Adv_z(numBlocksXY,threadsPerBlockXY,compute_stream,pitch,Nx,Ny,Nz,dz,d_u,d_Lu);
    ...
    PrintSummary("Burgers-3D GPU-WENO5", "Texture + Blocking", compute_timer, HtD_timer, DtH_timer, gflops, it, Nx, Ny, Nz);
